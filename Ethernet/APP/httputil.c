...
void proc_http(uint8_t s, uint8_t * buf)
{
  ...
  switch (http_request->METHOD)    
  {
    case METHOD_ERR :                              // 请求报文头错误*/
      ...
      send(s, (uint8_t *)http_response, strlen((int8_t const*)http_response));
      break;
    ...
    case METHOD_POST:                              // POST请求
      ...
      {
        cgi_ipconfig(http_request);                // 将配置信息写进单片机Flash
        make_cgi_response(5,(int8_t*)ConfigMsg.lip,tx_buf);          // 生成响应的文本部分        
        sprintf((char *)http_response,"HTTP/1.1 200 OK\r\nContent-Type: text/html\r\nContent-Length:%d\r\n\r\n%s",strlen(tx_buf),tx_buf);
                                                   // 发送http响应
